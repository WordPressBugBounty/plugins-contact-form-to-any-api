(function ($) {
	...
	$(document).ready(function () {
		...
		// Settings page: toggle "failure email" sub-rows when the checkbox changes
		var $failureEmailCheckbox = jQuery('#cf7_to_api_failure_email_enable');
		if ($failureEmailCheckbox.length) {
			$failureEmailCheckbox.on('change', function () {
				jQuery('.cf7_to_api_failure_email_row').toggle(this.checked);
			});
		}

		// Settings page: toggle "auto-delete days" row when the checkbox changes
		var $autoDeleteCheckbox = jQuery('#cf7_to_api_auto_delete_logs');
		if ($autoDeleteCheckbox.length) {
			$autoDeleteCheckbox.on('change', function () {
				jQuery('.cf7_to_api_auto_delete_days_row').toggle(this.checked);
			});
		}
